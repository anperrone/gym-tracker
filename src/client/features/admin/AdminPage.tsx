...
function CreateGlobalExerciseForm() {
  ...
  function submit(e: FormEvent) {
    ...
    if (trimmed === '') return;
    create.mutate({ name: trimmed, equipment }, { onSuccess: () => setName('') });
  }
    ...
    <Card className="flex items-center justify-between gap-2 p-3">
      {editing ? (
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-text-muted">{EQUIPMENT_LABELS[exercise.equipment]}</p>
        </div>
      )}
      ...
      <section className="space-y-3">
        ...
        {users.isPending ? (
          ...
          <ul aria-label="Utenti" className="space-y-2">
            {users.data?.map((u) => (
              <li key={u.id}>
                <UserRow user={u} isSelf={u.id === user.id} />
              </li>
            ))}
          </ul>
        )}
